...
export function TrackRecommendRow({ track, onSelect, showReason = false }) {
  ...
  return (
    <button
      ...
      className="flex w-full items-center gap-3 rounded-xl border border-zinc-900/10 bg-white px-3 py-2.5 text-left transition hover:border-accent/30 hover:bg-zinc-50 dark:border-white/10 dark:bg-white/5 dark:hover:border-accent/40 dark:hover:bg-white/[0.07]"
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {showReason && track.reason && (
          <p className="mt-0.5 truncate text-[11px] text-zinc-400">{track.reason}</p>
        )}
